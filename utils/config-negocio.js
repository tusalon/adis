...
// ============================================
const NEGOCIO_ID_POR_DEFECTO = '031e4d65-1cb3-4adb-87aa-3e8a38b3b7ab'; // ID de Adis
...
window.getTelefonoDuenno = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.telefono || '55065385';
};
...
window.getEmailNegocio = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.email || 'adishumara@nauta.com';
};
...
window.getMensajeBienvenida = async function() {
    const config = await window.cargarConfiguracionNegocio();
    return config?.mensaje_bienvenida || '¡Bienvenida a Adis!';
};
...
console.log('✅ config-negocio.js listo para Adis');
